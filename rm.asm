...
main(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++){
    ...
    if(unlink(argv[i]) < 0){
      ...
      66:	c7 44 24 04 6d 10 00 	movl   $0x106d,0x4(%esp)
      6d:	00 
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     458:	0f b6 80 14 15 00 00 	movzbl 0x1514(%eax),%eax
  ...
  for(i = 0; fmt[i]; i++){
    ...
    if(state == 0){
     ...
     5b5:	75 09                	jne    5c0 <printf+0x101>
          s = "(null)";
     5b7:	c7 45 f4 86 10 00 00 	movl   $0x1086,-0xc(%ebp)
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
     681:	a1 30 15 00 00       	mov    0x1530,%eax
...
free(void *ap)
{
  ...
  freep = p;
     ...
     74c:	a3 30 15 00 00       	mov    %eax,0x1530
}
     751:	c9                   	leave  
     752:	c3                   	ret    
...
morecore(uint nu)
{
  ...
  return freep;
     7a4:	a1 30 15 00 00       	mov    0x1530,%eax
}
     7a9:	c9                   	leave  
     7aa:	c3                   	ret    
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
     7c0:	a1 30 15 00 00       	mov    0x1530,%eax
     7c5:	89 45 f0             	mov    %eax,-0x10(%ebp)
     7c8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
     7cc:	75 23                	jne    7f1 <malloc+0x46>
    base.s.ptr = freep = prevp = &base;
     7ce:	c7 45 f0 28 15 00 00 	movl   $0x1528,-0x10(%ebp)
     7d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
     7d8:	a3 30 15 00 00       	mov    %eax,0x1530
     7dd:	a1 30 15 00 00       	mov    0x1530,%eax
     7e2:	a3 28 15 00 00       	mov    %eax,0x1528
    base.s.size = 0;
     7e7:	c7 05 2c 15 00 00 00 	movl   $0x0,0x152c
     7ee:	00 00 00 
  }
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p->s.size >= nunits){
     ...
     844:	a3 30 15 00 00       	mov    %eax,0x1530
      return (void*)(p + 1);
    ...
    if(p == freep)
     851:	a1 30 15 00 00       	mov    0x1530,%eax
